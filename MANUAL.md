...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/info` | name, firmware version, IP, chip ID, whether auth is required, static device-health facts (see below) |
...
| GET | `/api/log` | recent notable events (NTP, calibration, sensor recovery, config changes, boot) — see [Device log](#device-log--debug-console) |
| POST | `/api/debugcmd` | `{"cmd": "s"}` — runs one debug-console command over HTTP, same set as the USB console |
...
{"time":"19:24:13","date":"2026-07-18","dow":6,"rtcOk":true,
 ...
 "mqttConnected":false,"manualMode":false,
 "freeHeap":31992,"heapFragPct":2,"loopHz":18428}
```

`manualMode`/`freeHeap`/`heapFragPct`/`loopHz` are device-health/state fields
added 2026-07-24 — see [Device health](#device-health).
...
| `serialDebug` | false | USB debug console armed (sensor paused while armed + unplugged) |
| `autoReboot` | false | scheduled restart on/off — see [Device health](#device-health) |
| `autoRebootDays` | 7 | days between scheduled restarts (app offers 1/7/30 presets) |
| `autoRebootHour` | 4 | hour of day (0-23, device time) the scheduled restart fires |
...
The ESP8266 has no real task scheduler and so no "CPU load %" to report — the
closest useful equivalent, shown in **Settings → Device → Device health**:

| Field | Meaning |
|---|---|
| Free memory | RAM headroom. Steadily shrinking over days/weeks would mean a leak — a single reading swinging around minute to minute is normal and not the same thing. |
| Memory fragmentation | How chopped-up the free RAM is, as a %. A live, noisy snapshot (bounces with whatever the clock is doing at that instant) — watch free memory over time instead if actually worried. |
| Main loop rate | How many times/sec the firmware's main loop completes. The closest thing to "how busy is it" — drops while something's blocking (calibration, OTA, a slow config read), or under heavier HTTP request traffic. |
| CPU speed | Fixed setting (80/160MHz), not a live load. |
| Firmware storage used | Flash space the firmware occupies vs. what's left. |
| Last reboot cause | Power-on, crash/watchdog, or a software restart (e.g. after OTA). |

### Scheduled restart

Optional, **off by default** — an experiment to opt into, not a default
behavior. Memory fragmentation isn't realistically eliminable in code (any web
request handling does some varying-sized temporary allocation); a scheduled
restart just resets it to 0 instead. **Settings → Device → Scheduled restart**:
a toggle, a Day/Week/Month interval, and an hour-of-day slider (device time,
defaults to 4am — after the nightly NTP sync). Turning it on doesn't fire an
immediate restart; the schedule is anchored to the day it's turned on (or
retimed), so the first restart is properly a full interval away.

---

## Device log & debug console

**Settings → Device log** — a small in-RAM rolling log (40 entries, lost on
reboot) plus a command input, for debugging without a USB cable. Goes entirely
over WiFi/HTTP — it never touches the physical UART pins the sensor uses, so
it works regardless of sensor/debug-console mode.

**What's logged**: boot (with reset reason), NTP sync results, calibration
start/result, sensor auto-recovery, WiFi connect/disconnect, and any settings
change (which keys changed, not values — passwords live in the same payloads).
It is *not* a full mirror of the physical USB console's continuous output
(the per-second tick log, etc.) — those exist behind `serialDebug` specifically
because writing to Serial at all sends bytes out on the wire the sensor's RX
listens on, so most of that output is intentionally suppressed while the
sensor is active anyway.

**Commands** (`POST /api/debugcmd`, `{"cmd":"..."}`) mirror the same set the
physical console takes (see
[firmware_pio/SERIAL_COMMANDS.md](firmware_pio/SERIAL_COMMANDS.md)) — but
`runDebugCmd()` in `main.cpp` is a deliberately **separate implementation**
from `handleSerialDebug()`, not shared code, so this HTTP path can never
change how the physical console behaves.

Commands that put the display into **manual/test mode** (a bare number, `c`,
`x`, `n`, `p`) or write the RTC (`u`) ask for confirmation in the app before
sending, since there's no way to see the clock's physical face to catch a
mistake. Manual mode has **no auto-timeout on the clock's side** — the app's
own safety net is that leaving the Device log screen after entering it from
there automatically sends `r` to resume. `/api/state`'s `manualMode` field
also drives a persistent warning banner if it's ever left on.
...
| `POST /api/sensor/gate` | `{"gate":N, "motionThresholdDb":x, "microThresholdDb":y, "save":true}` — writes one gate without touching the other 15 |
...
**Per-gate tuning (Settings → Presence sensor)** — one card handles both
viewing and adjusting:

1. **"Show live gate signal"** turns Full data on (if it wasn't already) and
   loads the 16 gates' current thresholds in one step.
2. Opens **view-only**: live signal bars for every gate, with each threshold
   shown as a marker on the bar (visible, not draggable) — safe to just look
   without risk of nudging a value by accident.
3. An **Edit** switch in the same section unlocks dragging the markers, and
   reveals **Save this gate** (writes just that one gate, via
   `/api/sensor/gate`) and a bulk **Save to sensor** for everything changed.
   Both are disabled/hidden once there's nothing unsaved to write.

**Apply vs. Save to sensor**, for the separate **Range & timing** card (max
distance, disappearance delay): Apply sends the value live (the sensor's RAM)
so it's in effect immediately, but reverts if the sensor module itself loses
power. Save to sensor additionally commits it to the sensor's own flash, so it
survives that. This distinction is the sensor firmware's own design (flash has
limited write cycles) — the **Full data** toggle doesn't have this split
because the ESP always persists that one flag itself, regardless.
